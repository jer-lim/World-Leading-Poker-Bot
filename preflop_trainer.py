...
def get_benchmark():
	conn = HTTPSConnection(host)
	conn.request("GET", "/preflop/6/benchmark/" + socket.gethostname());
	response = conn.getresponse()
...
def do_benchmark(weights1, weights2, min_game):
	config = setup_config(max_round=max_round, initial_stack=initial_stack, small_blind_amount=smallblind_amount)
	# Register players
	config.register_player(name="agent1", algorithm=OurPlayerNoMwu(preflop_weights=weights1))
	config.register_player(name="agent2", algorithm=OurPlayerNoMwu(preflop_weights=weights2))
